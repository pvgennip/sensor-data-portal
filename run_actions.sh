...
docker-compose stop
docker-compose up -d --build apache2 php-fpm mysql phpmyadmin influx bower workspace
...
# angular app 
docker exec -it laradock_workspace_1 script /dev/null -c "cd public/webapp && bower install"
