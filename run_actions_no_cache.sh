...
docker-compose stop
docker-compose build --remove-orphans --no-cache apache2 php-fpm mysql phpmyadmin influx mosquitto spulmqttserver mqttwarn workspace
docker-compose up -d apache2 php-fpm mysql phpmyadmin influx mosquitto spulmqttserver mqttwarn workspace
