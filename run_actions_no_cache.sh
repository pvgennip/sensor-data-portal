...
docker-compose stop
docker-compose build --remove-orphans --no-cache apache2 php-fpm mysql phpmyadmin influx mosquitto spul-server mqttwarn workspace
docker-compose up -d apache2 php-fpm mysql phpmyadmin influx mosquitto spul-server mqttwarn workspace
