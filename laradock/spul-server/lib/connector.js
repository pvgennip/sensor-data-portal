...
var log = bunyan.default.createLogger({
...
// Configuration
var timestampPort = parseInt(process.env['SPUL_TS_PORT']) || 3333;
var payloadPort   = parseInt(process.env['SPUL_PORT']) || 5555;
...
var mqttHost 	  = process.env['MQTT_HOST'] || 'unknown.host';
...
// Timestamp server
var timestamps = net.default.createServer(function (socket) {
...
// Payload server
var total = 0;
var payloadServer = net.default.createServer(function (socket) {
	socket.setTimeout(socketTimeout, socket.destroy);
	...
	socket.on('data', function (buf) {
		...
		var client = mqtt.default.connect('mqtt://' + mqttHost, {
			clientId: deviceId,
			password: authToken,
			connectTimeout: 3000 });
